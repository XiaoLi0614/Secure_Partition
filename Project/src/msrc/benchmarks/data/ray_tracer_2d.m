function rays = ray_tracer_2d(x_s, y_s, x_d, y_d, M, N, d)
% rays = ray_tracer_2d(x_s, y_s, x_d, y_d, M, N, d)
% code can be modified for two different d's
%
% this code is used for finding the matrix, given source and detector
%
% @input:
% x_s, y_s: the coordinates of the source
% x_d, y_d: the coordinates of the detector
% M, N: number of pixels in x and y direction
% d: the bound of the image, image is in [-d,d]x[-d,d]
%
% @output:
...
%
% Computes lengths of ray-voxel intersections in a plane by tracing a ray from (x_s,y_s) to (x_d,y_d) through a grid of NxN pixels. The origin of the used coordinates system is in the center of the grid. 
...
%Initialization
rays = sparse(M,N);
% if we have different d, we also have different h
% h is the distance of the pixels in each direction
h = 2*d/M;
...
cos_ang_ray_h = x_sd/(sqrt(x_sd^2 + y_sd^2));
