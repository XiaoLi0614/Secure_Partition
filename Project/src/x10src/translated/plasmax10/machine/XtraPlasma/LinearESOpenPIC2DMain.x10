class PoissonSolver {
	public static global val PI4I: Double = (0.5 / 6.28318530717959);
	public static global val SQT2I: Double = 0.707106781186548;
	public static global val SQT2PI: Double = 0.797884560802865;
	public static global val GREENS_IDX: Int = 0;
	public static global val SHAPE_IDX: Int = 1;
	public static global val ELECX_IDX: Int = 2;
	public static global val ELECY_IDX: Int = 3;
	public static global val AR: Double = 0.912871;
	public global val x: Int;
	public global val y: Int;
	public global val xBits: Int;
	public global val yBits: Int;
	public global val ari: Double;
	public global val affp: Double;
	public global val fft: FourierTransform2D;
	public global val ffg: Array[Double]{(self.rank == 3)};
	public global val ffg2: Array[Double]{(self.rank == 2)};
	...
	public proto global def initTables(val fft: FourierTransform2D, val x: Int, val y: Int, val affp: Double): Array[Double]{(self.rank == 3)} {
	...
	public proto global def makeFFG(val table: Array[Double]{(self.rank == 3)}, val x: Int, val y: Int): Array[Double]{(self.rank == 3)} {
		val r1: Region{(self.rank == 1)} = Region.make([(0..(x - 1))]);
		val r2: Region{(self.rank == 1)} = Region.make([(0..y)]);
		val r3: Region{(self.rank == 1)} = Region.make([(0..3)]);
	...
	public proto global def makeFFG2(val table: Array[Double]{(self.rank == 3)}, val x: Int, val y: Int): Array[Double]{(self.rank == 2)} {
				...
				val val: Double = table(x, i, j);
{
	...
	public proto global def initGreensFnTable(val table: Array[Double]{(self.rank == 3)}, val fft: FourierTransform2D, val x: Int, val y: Int, val affp: Double): Void {
	...
	public proto global def initShapeFactorTable(val table: Array[Double]{(self.rank == 3)}, val fft: FourierTransform2D, val x: Int, val y: Int, val affp: Double): Void {
		val r1: Region{(self.rank == 1)} = Region.make([(0..((2 * y) - 1))]);
		val r2: Region{(self.rank == 1)} = Region.make([(0..((2 * x) - 1))]);
	...
	public proto global def initElectricFieldXTable(val table: Array[Double]{(self.rank == 3)}, val fft: FourierTransform2D, val x: Int, val y: Int, val affp: Double): Void {
		val r1: Region{(self.rank == 1)} = Region.make([(0..((2 * y) - 1))]);
		val r2: Region{(self.rank == 1)} = Region.make([(0..((2 * x) - 1))]);
	...
	public proto global def initElectricFieldYTable(val table: Array[Double]{(self.rank == 3)}, val fft: FourierTransform2D, val x: Int, val y: Int, val affp: Double): Void {
		val r1: Region{(self.rank == 1)} = Region.make([(0..((2 * y) - 1))]);
		val r2: Region{(self.rank == 1)} = Region.make([(0..((2 * x) - 1))]);
		val dElecX: Dist{(self.rank == 2)} = X10Util.distBlockStar2(r1, r2);
		val f: Array[Double]{(self.rank == 2)} = (Array.make[Double](dElecX));
	...
	public proto global def genTable1(val table: Array[Double]{(self.rank == 3)}, val ft: Array[Complex]{(self.rank == 2)}, val index: Int, val x: Int, val y: Int): Void {
		val N: Int = ((4 * x) * y);
		for (pl: Point{(self.rank == 1)} in Dist.makeUnique()) {
			if ((pl(0) <= x)) {
				...
				for ((i, j): Point{(self.rank == 2)} in Region.make([rLocalFT1, rLocalT2])) {
					table(i, j, index) = (N * ft(i, j).getReal());
				}
				if (here.isFirst()) {
					for ((i): Point{(self.rank == 1)} in Region.make([(1..(y - 1))])) {
	...
	public proto global def genTable2(val table: Array[Double]{(self.rank == 3)}, val ft: Array[Complex]{(self.rank == 2)}, val index: Int, val x: Int, val y: Int): Void {
		val N: Int = ((4 * x) * y);
		for (pl: Point{(self.rank == 1)} in Dist.makeUnique()) {
			if ((pl(0) <= x)) {
				val dFT: Dist{(self.rank == 2)} = ft.dist;
				...
				if (here.isFirst()) {
					for ((i1): Point{(self.rank == 1)} in Region.make([(1..(y - 1))])) {
						val i2: Int = ((2 * y) - i1);
	...
	public proto global def genTable3(val table: Array[Double]{(self.rank == 3)}, val ft: Array[Complex]{(self.rank == 2)}, val index: Int, val x: Int, val y: Int): Void {
		val N: Int = ((4 * x) * y);
		for (pl: Point{(self.rank == 1)} in Dist.makeUnique()) {
			if ((pl(0) <= x)) {
				val dFT: Dist{(self.rank == 2)} = ft.dist;
	...
	public global def makeForceArray(): Array[Complex]{(self.rank == 2)} {
		val dResult: Dist{(self.rank == 2)} = X10Util.distBlockStar1(Region.make([(0..((2 * y) - 1)), (0..(x - 1))]));
		return (Array.make[Complex](dResult));
	}

	public global def getForceCharge(val q: Array[Complex]{(self.rank == 2)}, val forceX: Array[Complex]{(self.rank == 2)}, val forceY: Array[Complex]{(self.rank == 2)}): Double {
	...
	public global def solveForceCharge(val q: Array[Complex]{(self.rank == 2)}, val forceX: Array[Complex]{(self.rank == 2)}, val forceY: Array[Complex]{(self.rank == 2)}): Double {
	...
	public proto global def potentialField(val r: Double, val ari: Double, val affp: Double): Double {
	...
	public proto global def particleShape(val r: Double, val ari: Double, val affp: Double): Double {
	...
	public proto global def radialElectricField(val r: Double, val ari: Double, val affp: Double): Double {
		val anorm: Double = (affp * PI4I);
		if ((ari > 0.0)) {
			if ((r == 0.0)) {
				return 0.0;
	...
	public proto global def errorFn(val x: Double): Double {
	...
	public global def expIntNL(val x: Double): Double {
	...
	public safe global def toString(): String {
...
class Complex {
	private global val real: Double;
	private global val imag: Double;
	...
	public global def getReal(): Double {
		return real;
	}

	public global def getImag(): Double {
		return imag;
	}

	public global def add(val n: Complex): Complex {
		return new Complex((getReal() + n.getReal()), (getImag() + n.getImag()));
	}

	public global def sub(val n: Complex): Complex {
		return new Complex((getReal() - n.getReal()), (getImag() - n.getImag()));
	}

	public global def mult(val n: Complex): Complex {
	...
	public global def multS(val r: Double): Complex {
		return new Complex((r * getReal()), (r * getImag()));
	}

	public global def getConjg(): Complex {
		return new Complex(getReal(), -getImag());
	}

	public global def multConjg(): Double {
		val a: Double = getReal();
		val b: Double = getImag();
		return (((a * a) + (b * b)));
	}

	public global def complexEquals(val c: Complex): Boolean {
...
class Random {
	private global var flag: Int;
	private global var r0: Double;
	private global var r1: Int;
	private global var r2: Int;
	private global var r4: Int;
	private global var r5: Int;
	private global var h1l: Double;
	private global var h1u: Double;
	private global var h2l: Double;
	...
	public global def nextNormal(): Double {
		var ret: Double = 0.0;
		if ((flag != 0)) {
			ret = r0;
			r0 = 0.0;
...
class X10Util {
	public static global val DEBUG: Boolean = true;
	public static global val ENABLE_CHECKS: Boolean = true;
	public static global val ERROR: Double = 1.0e-13;


	public static global def maxDouble(val d1: Double, val d2: Double): Double {
		return ((d1 >= d2))?d1:d2;
	}

	public static global def minDouble(val d1: Double, val d2: Double): Double {
		return ((d1 >= d2))?d2:d1;
	}

	public static global def maxInt(val i1: Int, val i2: Int): Int {
		return ((i1 >= i2))?i1:i2;
	}

	public static global def minInt(val i1: Int, val i2: Int): Int {
		return ((i1 >= i2))?i2:i1;
	}

	public static global def prependRegionRank2D(val r1: Region{(self.rank == 1)}, val r2: Region{(self.rank == 1)}): Region{(self.rank == 2)} {
		return extendRegionRank2D(r1, r2);
	}

	public static global def prependRegionRank3D(val r1: Region{(self.rank == 1)}, val r2: Region{(self.rank == 2)}): Region{(self.rank == 3)} {
		return Region.make([r1, getRank2D(r2, 0), getRank2D(r2, 1)]);
	}

	public static global def extendRegionRank2D(val r1: Region{(self.rank == 1)}, val r2: Region{(self.rank == 1)}): Region{(self.rank == 2)} {
		return Region.make([r1, r2]);
	}

	public static global def extendRegionRank3D(val r1: Region{(self.rank == 2)}, val r2: Region{(self.rank == 1)}): Region{(self.rank == 3)} {
		return Region.make([getRank2D(r1, 0), getRank2D(r1, 1), r2]);
	}

	public static global def prependDistRank2D(val r1: Region{(self.rank == 1)}, val d2: Dist{(self.rank == 1)}): Dist{(self.rank == 2)} {
	...
	public static global def prependDistRank3D(val r1: Region{(self.rank == 1)}, val d2: Dist{(self.rank == 2)}): Dist{(self.rank == 3)} {
	...
	public static global def extendDistRank2D(val d1: Dist{(self.rank == 1)}, val r2: Region{(self.rank == 1)}): Dist{(self.rank == 2)} {
		var dResult: Dist{(self.rank == 2)} = (((Region.makeEmpty(2)->here)) as Dist{(self.rank == 2)});
		for (pl: Point{(self.rank == 1)} in Dist.makeUnique()) {
			val p: Place = Dist.makeUnique()(pl);
	...
	public static global def extendDistRank3D(val d1: Dist{(self.rank == 2)}, val r2: Region{(self.rank == 1)}): Dist{(self.rank == 3)} {
		var dResult: Dist{(self.rank == 3)} = (((Region.makeEmpty(3)->here)) as Dist{(self.rank == 3)});
		for (pl: Point{(self.rank == 1)} in Dist.makeUnique()) {
			val p: Place = Dist.makeUnique()(pl);
	...
	public static global def distBlock(val r: Region{(self.rank == 1)}): Dist{(self.rank == 1)} {
		if ((r.min(0) != 0)) {
	...
	public static global def distBlockStar1(val r: Region{(self.rank == 2)}): Dist{(self.rank == 2)} {
		val r1: Region{(self.rank == 1)} = getRank2D(r, 0);
		val d1: Dist{(self.rank == 1)} = distBlock(r1);
		return extendDistRank2D(d1, getRank2D(r, 1));
	}

	public static global def distBlockStar2(val r1: Region{(self.rank == 1)}, val r2: Region{(self.rank == 1)}): Dist{(self.rank == 2)} {
		return distBlockStar1(Region.make([r1, r2]));
	}

	public static global def distStarBlock1(val r: Region{(self.rank == 2)}): Dist{(self.rank == 2)} {
		val r2: Region{(self.rank == 1)} = getRank2D(r, 1);
		val d2: Dist{(self.rank == 1)} = distBlock(r2);
		return prependDistRank2D(getRank2D(r, 0), d2);
	}

	public static global def distStarBlock2(val r1: Region{(self.rank == 1)}, val r2: Region{(self.rank == 1)}): Dist{(self.rank == 2)} {
		return distStarBlock1(Region.make([r1, r2]));
	}

	public static global def isDistBlockStar(val d: Dist{(self.rank == 2)}): Boolean {
		if (ENABLE_CHECKS) {
	...
	public static global def isDistStarBlock(val d: Dist{(self.rank == 2)}): Boolean {
		if (ENABLE_CHECKS) {
			val rd: Region{(self.rank == 2)} = d.region;
			val r1: Region{(self.rank == 1)} = getRank2D(rd, 0);
			val r2: Region{(self.rank == 1)} = getRank2D(rd, 1);
	...
	public static global def get2DDistRank(val d: Dist{(self.rank == 2)}, val n: Int): Dist{(self.rank == 1)} {
		if (((n > 2) || (n < 0))) {
	...
	public static global def transposeRegion(val r: Region{(self.rank == 2)}): Region{(self.rank == 2)} {
		return Region.make([getRank2D(r, 1), getRank2D(r, 0)]);
	}

	public static global def transposeDist(val d: Dist{(self.rank == 2)}): Dist{(self.rank == 2)} {
		var dResult: Dist{(self.rank == 2)} = (((Region.makeEmpty(2)->here)) as Dist{(self.rank == 2)});
		for (pl: Point{(self.rank == 1)} in Dist.makeUnique()) {
			val p: Place = Dist.makeUnique()(pl);
			val rLocal: Region{(self.rank == 2)} = (((d | p)).region as Region{(self.rank == 2)});
	...
	public static global def transposeBlockStar(val d: Dist{(self.rank == 2)}): Dist{(self.rank == 2)} {
		val rd: Region{(self.rank == 2)} = d.region;
		val r1: Region{(self.rank == 1)} = getRank2D(rd, 0);
		val r2: Region{(self.rank == 1)} = getRank2D(rd, 1);
	...
	public static global def pullBoolean(val b: Array[Boolean]{(self.rank == 1)}, val pt: Point{(self.rank == 1)}): Boolean {
			...
			val val: Boolean = b(pt);
{
	...
	public static global def pullComplex(val c: Array[Complex]{(self.rank == 1)}, val pt: Point{(self.rank == 1)}): Complex {
			...
			val val: Complex = c(pt);
{
	...
	public static global def pullInt(val i: Array[Int]{(self.rank == 1)}, val pt: Point{(self.rank == 1)}): Int {
			...
			val val: Int = i(pt);
{
	...
	public static global def pullDouble(val d: Array[Double]{(self.rank == 1)}, val pt: Point{(self.rank == 1)}): Double {
			...
			val val: Double = d(pt);
{
	...
	public static global def getRank2D(val r: Region{(self.rank == 2)}, val n: Int): Region{(self.rank == 1)} {
		return (r.projection(n) as Region{(self.rank == 1)});
	}

	public static global def getRank3D(val r: Region{(self.rank == 3)}, val n: Int): Region{(self.rank == 1)} {
		return (r.projection(n) as Region{(self.rank == 1)});
	}

	public static global def getLRank2D(val d: Dist{(self.rank == 2)}, val n: Int): Region{(self.rank == 1)} {
		return getPRank2D(d, here, n);
	}

	public static global def getLRank3D(val d: Dist{(self.rank == 3)}, val n: Int): Region{(self.rank == 1)} {
		return getPRank3D(d, here, n);
	}

	public static global def getPRank2D(val d: Dist{(self.rank == 2)}, val p: Place, val n: Int): Region{(self.rank == 1)} {
		return getRank2D(((d | p)).region, n);
	}

	public static global def getPRank3D(val d: Dist{(self.rank == 3)}, val p: Place, val n: Int): Region{(self.rank == 1)} {
		return getRank3D(((d | p)).region, n);
	}

	public static global def get1DLRegion(val d: Dist{(self.rank == 1)}): Region{(self.rank == 1)} {
		return (((d | here)).region as Region{(self.rank == 1)});
	}

	public static global def get1DPRegion(val d: Dist{(self.rank == 1)}, val p: Place): Region{(self.rank == 1)} {
		return (((d | p)).region as Region{(self.rank == 1)});
	}

	public static global def get2DLRegion(val d: Dist{(self.rank == 2)}): Region{(self.rank == 2)} {
		return (((d | here)).region as Region{(self.rank == 2)});
	}

	public static global def get2DPRegion(val d: Dist{(self.rank == 2)}, val p: Place): Region{(self.rank == 2)} {
		return (((d | p)).region as Region{(self.rank == 2)});
	}

	public static global def get3DLRegion(val d: Dist{(self.rank == 3)}): Region{(self.rank == 3)} {
		return (((d | here)).region as Region{(self.rank == 3)});
	}

	public static global def get3DPRegion(val d: Dist{(self.rank == 3)}, val p: Place): Region{(self.rank == 3)} {
		return (((d | p)).region as Region{(self.rank == 3)});
	}

	public static global def print(val s: String): Void {
		if (DEBUG) {
			Console.OUT.print(s);
		}
	}

	public static global def println(val s: String): Void {
		print(s);
		newline();
	}

	public static global def newline(): Void {
		print("\n");
	}
...
class FourierTransform2D {
	public global val maxBits: Int;
	public global val nBits1: Int;
	public global val nBits2: Int;
	public global val length1: Int;
	public global val length2: Int;
	public global val invN: Double;
	public global val tableBitReverse: Array[Int]{(self.rank == 1)};
	public global val tableSineCosine: Array[Complex]{(self.rank == 1)};
	...
	public global def transformC(val f: Array[Complex]{(self.rank == 2)}): Array[Complex]{(self.rank == 2)} {
		scrambleY(f);
		bitReverse(f, 0);
	...
	public global def transformD(val f: Array[Double]{(self.rank == 2)}): Array[Complex]{(self.rank == 2)} {
		return transformC(ArrayUtil.doubleToComplex(f));
	}

	public global def inverseTransformC(val f: Array[Complex]{(self.rank == 2)}): Array[Complex]{(self.rank == 2)} {
		bitReverse(f, 1);
	...
	public global def inverseTransformD(val f: Array[Double]{(self.rank == 2)}): Array[Complex]{(self.rank == 2)} {
		return inverseTransformC(ArrayUtil.doubleToComplex(f));
	}

	public proto global def initBitReverse(val nBits1: Int, val nBits2: Int): Array[Int]{(self.rank == 1)} {
	...
	public proto global def initSineCosine(val length1: Int, val length2: Int): Array[Complex]{(self.rank == 1)} {
	...
	public global def bitReverse(val f: Array[Complex]{(self.rank == 2)}, val rnk: Int): Void {
		...
		for (pl: Point{(self.rank == 1)} in Dist.makeUnique()) {
			...
			for ((j1): Point{(self.rank == 1)} in Region.make([(0..(r1Length - 1))])) {
	...
	public global def scrambleX(val f: Array[Complex]{(self.rank == 2)}): Void {
		...
		for (pl: Point{(self.rank == 1)} in Dist.makeUnique()) {
			...
			for (pt: Point{(self.rank == 2)} in Region.make([rLocal1, (nxhh..nxhh)])) {
	...
	public global def unscrambleX(val f: Array[Complex]{(self.rank == 2)}): Void {
		val nxh: Int = (length2 / 2);
	...
	public global def scrambleY(val f: Array[Complex]{(self.rank == 2)}): Void {
{
			for ((j1): Point{(self.rank == 1)} in Region.make([(1..(((length1 / 2)) - 1))])) {
	...
	public global def unscrambleY(val f: Array[Complex]{(self.rank == 2)}): Void {
{
			for ((j1): Point{(self.rank == 1)} in Region.make([(1..(((length1 / 2)) - 1))])) {
				val j2: Int = (length1 - j1);
	...
	public global def transformRank(val f: Array[Complex]{(self.rank == 2)}, val rnk: Int, val inverse: Boolean): Void {
		...
		for (exponent: Point{(self.rank == 1)} in Region.make([(0..(bits - 1))])) {
			...
			for (pl: Point{(self.rank == 1)} in Dist.makeUnique()) {
				for ((k): Point{(self.rank == 1)} in Region.make([(0..(km - 1))])) {
					...
					for ((j): Point{(self.rank == 1)} in Region.make([(0..(stride - 1))])) {
						val j1: Int = (j + lb);
						val j2: Int = (j + mb);
...
class LinearESOpenPIC2D {
	private static global val NUMBER_OF_XBITS: Int = 5;
	private static global val NUMBER_OF_YBITS: Int = 5;
	private static global val XLENGTH: Int = (1 << NUMBER_OF_XBITS);
	private static global val YLENGTH: Int = (1 << NUMBER_OF_YBITS);
	private static global val PARTICLE_XDENSITY: Int = 128;
	private static global val PARTICLE_YDENSITY: Int = 128;
	private static global val NPARTICLES: Int = (PARTICLE_XDENSITY * PARTICLE_YDENSITY);
	private static global val DELTA_TIME: Double = 0.2000000e+00;
	private static global val ELECTRON_CHARGE: Double = -1.0;
	private static global val THERMAL_XVELOCITY: Double = 1.0;
	private static global val THERMAL_YVELOCITY: Double = 1.0;
	private static global val BOUNDARY_CONDITION: Int = 2;
	private static global val TIME_STEPS: Int = 325;
	private static global val SORT_TIME: Int = 50;
	private static global val IMBALANCE: Double = 0.08;
	private static global val MOVE_FIELD_TO_DIST_MAX_ROWS: Int = ((4 * YLENGTH) / Place.MAX_PLACES);
	private static global val MOVE_PARTICLES_MAX: Int = ((4 * NPARTICLES) / Place.MAX_PLACES);
	private static global val MOVE_PARTICLES_MAX_BUFFER: Int = ((4 * NPARTICLES) / Place.MAX_PLACES);
	private global val r: Random;
	private global val solver: PoissonSolver;
	private global var particles: Array[Particle]{(self.rank == 1)};
	private global var backgroundCharge: Array[Double]{(self.rank == 2)};
	private global var totalTime: Long;
	private global var initTime: Long;
	private global var depositTime: Long;
	private global var redistTime: Long;
	private global var solverTime: Long;
	private global var pushTime: Long;
	private global var moveTime: Long;
	...
	public global def initialize(): Void {
	...
	public global def genParticlePosition(val np: Int): Array[Double]{(self.rank == 2)} {
		val X: Int = 0;
		val Y: Int = 1;
	...
	public global def genParticleVelocity(val n: Int): Array[Double]{(self.rank == 2)} {
		val X: Int = 0;
		val Y: Int = 1;
					...
					val vy: Double = (THERMAL_YVELOCITY * r.nextNormal());
{
	...
	public global def genParticles(val n: Int, val position: Array[Double]{(self.rank == 2)}, val velocity: Array[Double]{(self.rank == 2)}): Array[Particle]{(self.rank == 1)} {
	...
	public global def genInitialDist(val particles: Array[Particle]{(self.rank == 1)}): Dist{(self.rank == 2)} {
		val MAX_PLACES: Int = Place.MAX_PLACES;
		val edgely: Double = ((BOUNDARY_CONDITION == 2))?1.0:0.0;
			...
			val r: Region{(self.rank == 1)} = (yRegions(pl) as Region{(self.rank == 1)});
{
	...
	public global def linearDensityFn(val x: Double, val anlx: Double, val anxi: Double, val shift: Double): Double {
	...
	public global def linearDensityFnIntegral(val x: Double, val anlx: Double, val anxi: Double, val shift: Double): Double {
	...
	public global def stepTime(val ionBackground: Array[Double]{(self.rank == 2)}): Array[Double]{(self.rank == 1)} {
	...
	public global def linearDensity(val x: Double, val anlx: Double, val anxi: Double, val shift: Double): Double {
		return (1.0 + (anlx * (((x * anxi) - shift))));
	}

	public global def linearDensityIntegral(val x: Double, val anlx: Double, val anxi: Double, val shift: Double): Double {
		return ((anxi == 0))?x:(x + (((0.5 * anlx) * x) * (((x * anxi) - (2.0 * shift)))));
	}

	public global def makeGuards(val dData: Dist{(self.rank == 2)}): Array[Double]{(self.rank == 2)} {
		var dResult: Dist{(self.rank == 2)} = (((Region.makeEmpty(2)->here)) as Dist{(self.rank == 2)});
		for (pl: Point{(self.rank == 1)} in Dist.makeUnique()) {
			val p: Place = Dist.makeUnique()(pl);
	...
	public global def depositCharge(val particles: Array[Particle]{(self.rank == 1)}, val fieldCharge: Array[Double]{(self.rank == 2)}, val guards: Array[Double]{(self.rank == 2)}, val charge: Double): Void {
		for (pl: Point{(self.rank == 1)} in Dist.makeUnique()) {
			...
			}
{
	...
	public global def addGuards(val data: Array[Double]{(self.rank == 2)}, val guards: Array[Double]{(self.rank == 2)}): Void {
				...
				})) as Array[Double]{(self.rank == 2)});
{
	...
	public global def doubleDimensions(val data: Array[Double]{(self.rank == 2)}): Array[Double]{(self.rank == 2)} {
			...
			})) as Array[Double]{(self.rank == 2)});
{
	...
	public global def moveFieldToUniform(val data: Array[Double]{(self.rank == 2)}): Array[Double]{(self.rank == 2)} {
		val rData: Region{(self.rank == 2)} = data.region;
		return moveFieldToDist(data, X10Util.distBlockStar1(rData));
	}

	public global def moveFieldToDist(val data: Array[Double]{(self.rank == 2)}, val dDst: Dist{(self.rank == 2)}): Array[Double]{(self.rank == 2)} {
					...
					})) as Array[Double]{(self.rank == 2)});
{
			...
			for ((pl): Point{(self.rank == 1)} in Dist.makeUnique()) {
				...
				if ((outRt(pl) > 0)) {
					...
					for ((i, j): Point{(self.rank == 2)} in Region.make([(0..(rsize - 1)), rRank2])) {
				...
				for (pt: Point{(self.rank == 3)} in Region.make([(pl..pl), (0..(inLt(pl) - 1)), rRank2])) {
					...
					})) as Array[Double]{(self.rank == 2)});
{
			...
			for ((pl): Point{(self.rank == 1)} in Dist.makeUnique()) {
				...
				for ((i, j): Point{(self.rank == 2)} in Region.make([(0..(inRt(pl) - 1)), rRank2])) {
		...
		for ((pl): Point{(self.rank == 1)} in Dist.makeUnique()) {
			...
			for ((i, j): Point{(self.rank == 2)} in Region.make([(0..(sizes(pl) - 1)), rRank2])) {
	...
	public global def cropField(val data: Array[Double]{(self.rank == 2)}, val guards: Array[Double]{(self.rank == 2)}): Array[Double]{(self.rank == 2)} {
				...
				})) as Array[Double]{(self.rank == 2)});
{
	...
	public global def loadGuards(val data: Array[Double]{(self.rank == 2)}, val guards: Array[Double]{(self.rank == 2)}): Void {
		for (pl: Point{(self.rank == 1)} in Dist.makeUnique()) {
			if (!here.isFirst()) {
				val dGuard: Dist{(self.rank == 2)} = guards.dist;
				...
				})) as Array[Double]{(self.rank == 2)});
{
	...
	public global def makeInfoArray(): Array[Int]{(self.rank == 1)} {
	...
	public global def moveParticles(val particles: Array[Particle]{(self.rank == 1)}, val dFieldSpace: Dist{(self.rank == 2)}, val info: Array[Int]{(self.rank == 1)}): Array[Particle]{(self.rank == 1)} {
		...
		var iter: Int = 2;
		var nter: Int = 0;
		for (pt: Point{(self.rank == 1)} in Region.make([(0..6)])) {
			info(pt) = 0;
		}
			...
			})) as Array[Particle]{(self.rank == 1)});
{
	...
	private global def packParticleArray(val part: Array[Particle]{(self.rank == 2)}, val nParticles: Array[Int]{(self.rank == 1)}): Array[Particle]{(self.rank == 1)} {
		var lb: Int = 0;
		var dResult: Dist{(self.rank == 1)} = (((Region.makeEmpty(1)->here)) as Dist{(self.rank == 1)});
	...
	public global def pushParticles(val particles: Array[Particle]{(self.rank == 1)}, val forceChargeX: Array[Double]{(self.rank == 2)}, val fcGuardX: Array[Double]{(self.rank == 2)}, val forceChargeY: Array[Double]{(self.rank == 2)}, val fcGuardY: Array[Double]{(self.rank == 2)}): Double {
		var lx: Double = 0.0;
		var ly: Double = 0.0;
		var rx: Double = 0.0;
		var ry: Double = 0.0;
	...
	private global def makeLocalSpace(val data: Array[Double]{(self.rank == 2)}, val guards: Array[Double]{(self.rank == 2)}): Array[Double]{(self.rank == 2)} {
		val dData: Dist{(self.rank == 2)} = data.dist;
		val dGuard: Dist{(self.rank == 2)} = guards.dist;
		val rLocalD: Region{(self.rank == 2)} = X10Util.get2DLRegion(dData);
		...
		val result: Array[Double]{(self.rank == 2)} = (Array.make[Double](dResult));
{
	...
	public global def countParticles(val particles: Array[Particle]{(self.rank == 1)}, val dField: Dist{(self.rank == 2)}): Array[Int]{(self.rank == 1)} {
	...
	public global def repartition(val particleCount: Array[Int]{(self.rank == 1)}, val dField: Dist{(self.rank == 2)}, val countAve: Double): Dist{(self.rank == 2)} {
		val MAX: Int = 1024;
		val dUnique: Dist{(self.rank == 1)} = (Dist.makeUnique() as Dist{(self.rank == 1)});
					...
					val nGrids: Int = sendGrids(pl);
{
			...
			for ((pl): Point{(self.rank == 1)} in Dist.makeUnique()) {
				...
				if (!here.isFirst()) {
					...
					for (pt: Point{(self.rank == 2)} in Region.make([(pl..pl), (0..(gsize - 1))])) {
			...
			val edge: Double = border(pl);
{
	...
	public global def sortParticles(val particles: Array[Particle]{(self.rank == 1)}, val dField: Dist{(self.rank == 2)}): Array[Particle]{(self.rank == 1)} {
	...
	public global def parallelPrefix(val data: Array[Int]{(self.rank == 1)}): Array[Int]{(self.rank == 1)} {
...
class Particle {
	private global val x: Double;
	private global val y: Double;
	private global val xVel: Double;
	private global val yVel: Double;
	private global val pid: Int;
	...
	public global def getX(): Double {
		return x;
	}

	public global def getY(): Double {
		return y;
	}

	public global def getXVel(): Double {
		return xVel;
	}

	public global def getYVel(): Double {
		return yVel;
	}

	public global def setX(val x: Double): Particle {
		return new Particle(x, y, xVel, yVel, pid);
	}

	public global def setY(val y: Double): Particle {
		return new Particle(x, y, xVel, yVel, pid);
	}

	public global def setXVel(val xVel: Double): Particle {
		return new Particle(x, y, xVel, yVel, pid);
	}

	public global def setYVel(val yVel: Double): Particle {
		return new Particle(x, y, xVel, yVel, pid);
	}

	public global def setPos(val x: Double, val y: Double): Particle {
		return new Particle(x, y, xVel, yVel, pid);
	}

	public global def setVel(val xVel: Double, val yVel: Double): Particle {
		return new Particle(x, y, xVel, yVel, pid);
	}
	...
	public global def particleEquals(val p: Particle): Boolean {
...
class ArrayUtil {
	...
	public static global def doubleToComplex(val src: Array[Double]{(self.rank == 2)}): Array[Complex]{(self.rank == 2)} {
	...
	public static global def complexToDouble(val src: Array[Complex]{(self.rank == 2)}): Array[Double]{(self.rank == 2)} {
		val rSrc: Region{(self.rank == 2)} = src.region;
		val rS2: Region{(self.rank == 1)} = X10Util.getRank2D(rSrc, 1);
	...
	public static global def transpose(val data: Array[Complex]{(self.rank == 2)}): Array[Complex]{(self.rank == 2)} {
