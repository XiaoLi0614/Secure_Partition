...
class PoissonSolver {
	...
	public val ffg: DistArray[Double]{(self.rank == 3)};
	public val ffg2: DistArray[Double]{(self.rank == 2)};
	...
	public def this(val xBits: Int, val yBits: Int, val affp: Double) {
		...
		y = y0;
		ari = 0;
		val fft0 <: FourierTransform2D = new FourierTransform2D((yBits + 1), (xBits + 1));
		fft = fft0;
		val tables <: DistArray[Double]{(self.rank == 3)} = initTables(fft0, x0, y0, affp);
	...
	@NonEscaping final
	public def initTables(val fft: FourierTransform2D, val x: Int, val y: Int, val affp: Double): DistArray[Double]{(self.rank == 3)} {
		val r1 <: Region{(self.rank == 1)} = (0..(x - 1));
		val r2 <: Region{(self.rank == 1)} = (0..y);
		val r3 <: Region{(self.rank == 1)} = (0..3);
		val dTable1 <: Dist{(self.rank == 3)} = X10Util.extendDistRank3D(X10Util.distBlockStar2(r1, r2), r3);
		val first <: Place = Place.FIRST_PLACE;
		val dTable2 <: Dist{(self.rank == 3)} = ((((x..x) * (0..y) * (0..3)->first)) as Dist{(self.rank == 3)});
		val dTable <: Dist{(self.rank == 3)} = (dTable1 || dTable2);
		val table <: DistArray[Double]{(self.rank == 3)} = (DistArray.make[Double](dTable));
	...
	@NonEscaping final
	public def makeFFG(val table: DistArray[Double]{(self.rank == 3)}, val x: Int, val y: Int): DistArray[Double]{(self.rank == 3)} {
		val r1 <: Region{(self.rank == 1)} = (0..(x - 1));
		val r2 <: Region{(self.rank == 1)} = (0..y);
		val r3 <: Region{(self.rank == 1)} = (0..3);
		val dResult <: Dist{(self.rank == 3)} = X10Util.extendDistRank3D(X10Util.distBlockStar2(r1, r2), r3);
		return (DistArray.make[Double](dResult, (pt:Point{(self.rank == 3)}) => {
			return table(pt);
		}));
	}

	@NonEscaping final
	public def makeFFG2(val table: DistArray[Double]{(self.rank == 3)}, val x: Int, val y: Int): DistArray[Double]{(self.rank == 2)} {
		val rResult <: Region{(self.rank == 2)} = (0..y) * (0..3);
		return ((DistArray.make[Double](rResult-> here, ([i, j]:Point{(self.rank == 2)}) => {
			val pHere <: Place = here;
			val dDst <: Dist{(self.rank == 1)} = ((0..0)->here);
			val rcv <: DistArray[Double]{(self.rank == 1)} = (DistArray.make[Double](dDst));
{
				val thaValue <: Double = table(x, i, j);

					rcv(0) = thaValue;
			}			return rcv(0);
		})) as DistArray[Double]{(self.rank == 2)});
	}

	@NonEscaping final
	public def initGreensFnTable(val table: DistArray[Double]{(self.rank == 3)}, val fft: FourierTransform2D, val x: Int, val y: Int, val affp: Double) {
		val r1 <: Region{(self.rank == 1)} = (0..((2 * y) - 1));
		val r2 <: Region{(self.rank == 1)} = (0..((2 * x) - 1));
		val dGreen <: Dist{(self.rank == 2)} = X10Util.distBlockStar2(r1, r2);
		val f <: DistArray[Double]{(self.rank == 2)} = (DistArray.make[Double](dGreen));
		val invRadius <: Double = ((AR > 0.0))?(1.0 / AR):0.0;
		val N <: Double = ((4.0 * x) * y);
		for (pl: Point{(self.rank == 1)} in Dist.makeUnique()) 
			if ((pl(0) <= (2 * y))) {
{
					val rLocal <: Region{(self.rank == 2)} = X10Util.get2DLRegion(dGreen);
					for (p: Point{(self.rank == 2)} in rLocal) {
		...
		val ft <: DistArray[Complex]{(self.rank == 2)} = fft.inverseTransformD(f);
		genTable1(table, ft, GREENS_IDX, x, y);
	}

	@NonEscaping final
	public def initShapeFactorTable(val table: DistArray[Double]{(self.rank == 3)}, val fft: FourierTransform2D, val x: Int, val y: Int, val affp: Double) {
		val r1 <: Region{(self.rank == 1)} = (0..((2 * y) - 1));
		val r2 <: Region{(self.rank == 1)} = (0..((2 * x) - 1));
		val dShape <: Dist{(self.rank == 2)} = X10Util.distBlockStar2(r1, r2);
		val f <: DistArray[Double]{(self.rank == 2)} = (DistArray.make[Double](dShape));
		val invRadius <: Double = ((AR > 0.0))?(1.0 / AR):0.0;
		val N <: Double = ((4 * x) * y);
		for (pl: Point{(self.rank == 1)} in Dist.makeUnique()) 
			if ((pl(0) <= (2 * y))) {
{
	...
	@NonEscaping final
	public def initElectricFieldXTable(val table: DistArray[Double]{(self.rank == 3)}, val fft: FourierTransform2D, val x: Int, val y: Int, val affp: Double) {
		val r1 <: Region{(self.rank == 1)} = (0..((2 * y) - 1));
		val r2 <: Region{(self.rank == 1)} = (0..((2 * x) - 1));
		val dElecX <: Dist{(self.rank == 2)} = X10Util.distBlockStar2(r1, r2);
		val f <: DistArray[Double]{(self.rank == 2)} = (DistArray.make[Double](dElecX));
		val invRadius <: Double = ((AR > 0.0))?(1.0 / AR):0.0;
		val N <: Double = ((4 * x) * y);
		for (pl: Point{(self.rank == 1)} in Dist.makeUnique()) 
			if ((pl(0) <= (2 * y))) {
{
					...
					for (p: Point{(self.rank == 2)} in rLocal) {
						...
						val thaValue <: Double = radialElectricField(r, invRadius, affp);
						f(p) = ((r > 0.0))?(thaValue * ((x2 / r))):thaValue;
					}
				}			}
		val ft <: DistArray[Complex]{(self.rank == 2)} = fft.inverseTransformD(f);
	...
	@NonEscaping final
	public def initElectricFieldYTable(val table: DistArray[Double]{(self.rank == 3)}, val fft: FourierTransform2D, val x: Int, val y: Int, val affp: Double) {
		val r1 <: Region{(self.rank == 1)} = (0..((2 * y) - 1));
		val r2 <: Region{(self.rank == 1)} = (0..((2 * x) - 1));
		val dElecX <: Dist{(self.rank == 2)} = X10Util.distBlockStar2(r1, r2);
		val f <: DistArray[Double]{(self.rank == 2)} = (DistArray.make[Double](dElecX));
			...
			if ((pl(0) <= (2 * y))) {
{
					...
					for (p: Point{(self.rank == 2)} in rLocal) {
						...
						val thaValue <: Double = radialElectricField(r, invRadius, affp);
						f(p) = ((r > 0.0))?(thaValue * ((y1 / r))):thaValue;
					}
				}			}
		val ft <: DistArray[Complex]{(self.rank == 2)} = fft.inverseTransformD(f);
	...
	@NonEscaping final
	public def genTable1(val table: DistArray[Double]{(self.rank == 3)}, val ft: DistArray[Complex]{(self.rank == 2)}, val index: Int, val x: Int, val y: Int) {
		val N <: Int = ((4 * x) * y);
		for (pl: Point{(self.rank == 1)} in Dist.makeUnique()) 
			if ((pl(0) <= x)) {
{
					...
					for ([i, j]: Point{(self.rank == 2)} in (0..rLocalFT1) * (0..rLocalT2)) 
						table(i, j, index) = (N * ft(i, j).getReal());
					if (here.isFirst()) {
{
							for ([i]: Point{(self.rank == 1)} in (1..(y - 1))) 
	...
	@NonEscaping final
	public def genTable2(val table: DistArray[Double]{(self.rank == 3)}, val ft: DistArray[Complex]{(self.rank == 2)}, val index: Int, val x: Int, val y: Int) {
		val N <: Int = ((4 * x) * y);
		for (pl: Point{(self.rank == 1)} in Dist.makeUnique()) 
			if ((pl(0) <= x)) {
{
					...
					val rLocalFT1 <: Region{(self.rank == 1)} = X10Util.getLRank2D(dFT, 0);
					val rLocalT2 <: Region{(self.rank == 1)} = X10Util.getLRank3D(dTable, 1);
					for ([i]: Point{(self.rank == 1)} in rLocalFT1) 
						if ((i > 0)) {

								for ([j]: Point{(self.rank == 1)} in rLocalT2) 
									table(i, j, index) = (N * ft(i, j).getImag());
						}					if (here.isFirst()) {
{
							for ([i1]: Point{(self.rank == 1)} in (1..(y - 1))) {
								val i2 <: Int = ((2 * y) - i1);
	...
	@NonEscaping final
	public def genTable3(val table: DistArray[Double]{(self.rank == 3)}, val ft: DistArray[Complex]{(self.rank == 2)}, val index: Int, val x: Int, val y: Int) {
		val N <: Int = ((4 * x) * y);
		for (pl: Point{(self.rank == 1)} in Dist.makeUnique()) 
			if ((pl(0) <= x)) {
{
					...
					for ([i]: Point{(self.rank == 1)} in rLocalFT1) 
						if ((i > 0)) {
{
								for ([j]: Point{(self.rank == 1)} in (1..(y - 1))) 
									table(i, j, index) = (N * ft(i, j).getImag());
					...
					if (here.isFirst()) {
{
	...
	public def makeForceArray(): DistArray[Complex]{(self.rank == 2)} {
		val dResult <: Dist{(self.rank == 2)} = X10Util.distBlockStar1((0..((2 * y) - 1)) * (0..(x - 1)));
		return (DistArray.make[Complex](dResult));
	}

	public def getForceCharge(val q: DistArray[Complex]{(self.rank == 2)}, val forceX: DistArray[Complex]{(self.rank == 2)}, val forceY: DistArray[Complex]{(self.rank == 2)}): Double {
		val dForce <: Dist{(self.rank == 2)} = X10Util.distBlockStar1((0..(x - 1)) * (0..((2 * y) - 1)));
		val fxfft <: DistArray[Complex]{(self.rank == 2)} = (DistArray.make[Complex](dForce));
		val fyfft <: DistArray[Complex]{(self.rank == 2)} = (DistArray.make[Complex](dForce));
		val qfft <: DistArray[Complex]{(self.rank == 2)} = fft.inverseTransformC(q);
		val we <: Double = solveForceCharge(qfft, fxfft, fyfft);
		val fx <: DistArray[Complex]{(self.rank == 2)} = fft.transformC(fxfft);
		val fy <: DistArray[Complex]{(self.rank == 2)} = fft.transformC(fyfft);
		for (pt: Point{(self.rank == 2)} in forceX) {
			forceX(pt) = fx(pt);
			forceY(pt) = fy(pt);
		}
		return we;
	...
	public def solveForceCharge(val q: DistArray[Complex]{(self.rank == 2)}, val forceX: DistArray[Complex]{(self.rank == 2)}, val forceY: DistArray[Complex]{(self.rank == 2)}): Double {
		val dUnique <: Dist{(self.rank == 1)} = (Dist.makeUnique() as Dist{(self.rank == 1)});
		val energy <: DistArray[Double]{(self.rank == 1)} = (DistArray.make[Double](dUnique, (pt:Point{(self.rank == 1)}) => {
			return 0.0;
		}));
		for (pl: Point{(self.rank == 1)} in Dist.makeUnique()) 
			if ((pl(0) <= x)) {
						...
						if ((i > 0)) {
{
								...
								for ([j1]: Point{(self.rank == 1)} in (1..(y - 1))) {
									val j2 <: Int = ((2 * y) - j1);
					...
					if (here.isFirst()) {
{
	...
	@NonEscaping final
	public def potentialField(val r: Double, val ari: Double, val affp: Double): Double {
	...
	@NonEscaping final
	public def particleShape(val r: Double, val ari: Double, val affp: Double): Double {
	...
	@NonEscaping final
	public def radialElectricField(val r: Double, val ari: Double, val affp: Double): Double {
		val anorm <: Double = (affp * PI4I);
		if ((ari > 0.0)) {
	...
	@NonEscaping final
	public def errorFn(val x: Double): Double {
		...
		if ((flag != 0)) {
{
			...
			var temp: Double = Math.sqrt((-2.0 * Math.log((((((r1 + (r2 * asc))) * asc) as Double) as Double))));
			isc = 65536;
			asc = isc;
			bsc = (asc * asc);
			i1 = (r4 - (((r4 / isc)) * isc));
...
class X10Util {
	...
	public static def prependRegionRank3D(val r1: Region{(self.rank == 1)}, val r2: Region{(self.rank == 2)}): Region{(self.rank == 3)} {
		return (0..r1) * (0..getRank2D(r2, 0)) * (0..getRank2D(r2, 1));
	}
	...
	public static def extendRegionRank3D(val r1: Region{(self.rank == 2)}, val r2: Region{(self.rank == 1)}): Region{(self.rank == 3)} {
		return (0..getRank2D(r1, 0)) * (0..getRank2D(r1, 1)) * (0..r2);
	}
	...
	public static def distBlock(val r: Region{(self.rank == 1)}): Dist{(self.rank == 1)} {
		...
		for (pl: Point{(self.rank == 1)} in Dist.makeUnique()) {
			val p <: Place = Dist.makeUnique()(pl);
			var ext: Dist{(self.rank == 1)} = (((((pl(0) * blockSize)..(((((pl(0) + 1)) * blockSize)) - 1))->p)) as Dist{(self.rank == 1)});
			dResult = (dResult || ext);
		}
		var diff: Int = (r.size() - (blockSize * Place.MAX_PLACES));
		if (((blockSize * Place.MAX_PLACES) != r.size())) {
{
				var l: Place = Place.place((Place.MAX_PLACES - 1));
				var ext: Dist{(self.rank == 1)} = (((((r.size() - diff)..(r.size() - 1))->l)) as Dist{(self.rank == 1)});
				dResult = (dResult || ext);
			}		}
		return dResult;
	}
	...
	public static def distBlockStar2(val r1: Region{(self.rank == 1)}, val r2: Region{(self.rank == 1)}): Dist{(self.rank == 2)} {
		return distBlockStar1((0..r1) * (0..r2));
	}
	...
	public static def distStarBlock2(val r1: Region{(self.rank == 1)}, val r2: Region{(self.rank == 1)}): Dist{(self.rank == 2)} {
		return distStarBlock1((0..r1) * (0..r2));
	}
		...
		if (ENABLE_CHECKS) {
{
	...
	public static def transposeRegion(val r: Region{(self.rank == 2)}): Region{(self.rank == 2)} {
		return (0..getRank2D(r, 1)) * (0..getRank2D(r, 0));
	}
	...
	public static def pullBoolean(val b: DistArray[Boolean]{(self.rank == 1)}, val pt: Point{(self.rank == 1)}): Boolean {
		if (!b.region.contains(pt)) {
{
		...
		val pHere <: Place = here;
		val dDst <: Dist{(self.rank == 1)} = ((0..0)->here);
		val result <: DistArray[Boolean]{(self.rank == 1)} = (DistArray.make[Boolean](dDst));
{
			val thaValue <: Boolean = b(pt);

				result(0) = thaValue;
		}		return result(0);
	}

	public static def pullComplex(val c: DistArray[Complex]{(self.rank == 1)}, val pt: Point{(self.rank == 1)}): Complex {
		if (!c.region.contains(pt)) {
{
		...
		val pHere <: Place = here;
		val dDst <: Dist{(self.rank == 1)} = ((0..0)->here);
		val result <: DistArray[Complex]{(self.rank == 1)} = (DistArray.make[Complex](dDst));
{
			val thaValue <: Complex = c(pt);

				result(0) = thaValue;
		}		return result(0);
	}

	public static def pullInt(val i: DistArray[Int]{(self.rank == 1)}, val pt: Point{(self.rank == 1)}): Int {
		if (!i.region.contains(pt)) {
{
		...
		val pHere <: Place = here;
		val dDst <: Dist{(self.rank == 1)} = ((0..0)->here);
		val result <: DistArray[Int]{(self.rank == 1)} = (DistArray.make[Int](dDst));
{
			val thaValue <: Int = i(pt);

				result(0) = thaValue;
		}		return result(0);
	}

	public static def pullDouble(val d: DistArray[Double]{(self.rank == 1)}, val pt: Point{(self.rank == 1)}): Double {
		if (!d.region.contains(pt)) {
{
		...
		val pHere <: Place = here;
		val dDst <: Dist{(self.rank == 1)} = ((0..0)->here);
		val result <: DistArray[Double]{(self.rank == 1)} = (DistArray.make[Double](dDst));
{
			val thaValue <: Double = d(pt);
	...
	public static def print(val s: String) {
		if (DEBUG) {

				Console.OUT.print(s);
		}	}

	public static def println(val s: String) {
		print(s);
		newline();
	}

	public static def newline() {
		print("\n");
	}
...
class FourierTransform2D {
	...
	public val tableBitReverse: DistArray[Int]{(self.rank == 1)};
	public val tableSineCosine: DistArray[Complex]{(self.rank == 1)};
	...
	public def transformC(val f: DistArray[Complex]{(self.rank == 2)}): DistArray[Complex]{(self.rank == 2)} {
		scrambleY(f);
		bitReverse(f, 0);
		transformRank(f, 0, false);
		val g <: DistArray[Complex]{(self.rank == 2)} = ArrayUtil.transpose(f);
		scrambleX(g);
		bitReverse(g, 1);
	...
	public def transformD(val f: DistArray[Double]{(self.rank == 2)}): DistArray[Complex]{(self.rank == 2)} {
		return transformC(ArrayUtil.doubleToComplex(f));
	}

	public def inverseTransformC(val f: DistArray[Complex]{(self.rank == 2)}): DistArray[Complex]{(self.rank == 2)} {
		bitReverse(f, 1);
	...
	public def inverseTransformD(val f: DistArray[Double]{(self.rank == 2)}): DistArray[Complex]{(self.rank == 2)} {
		return inverseTransformC(ArrayUtil.doubleToComplex(f));
	}

	@NonEscaping final
	public def initBitReverse(val nBits1: Int, val nBits2: Int): DistArray[Int]{(self.rank == 1)} {
		val maxBits <: Int = X10Util.maxInt((nBits2 - 1), nBits1);
		val r <: Region{(self.rank == 1)} = (0..(((1 << maxBits)) - 1));
		return ((DistArray.make[Int](r-> here, (i:Point{(self.rank == 1)}) => {
			val i0 <: Int = i(0);
	...
	@NonEscaping final
	public def initSineCosine(val length1: Int, val length2: Int): DistArray[Complex]{(self.rank == 1)} {
		val maxLength <: Int = X10Util.maxInt(length1, length2);
		val PI2 <: Double = 6.28318530717959;
		val k <: Double = (PI2 / maxLength);
		val r <: Region{(self.rank == 1)} = (0..(((maxLength / 2)) - 1));
		return ((DistArray.make[Complex](r-> here, (i:Point{(self.rank == 1)}) => {
			val theta <: Double = (k * i(0));
			return new Complex(Math.cos(theta), -Math.sin(theta));
		})) as DistArray[Complex]{(self.rank == 1)});
	}

	public def bitReverse(val f: DistArray[Complex]{(self.rank == 2)}, val rnk: Int) {
		...
		for (pl: Point{(self.rank == 1)} in Dist.makeUnique()) {
			...
			for ([j1]: Point{(self.rank == 1)} in (0..(r1Length - 1))) {
	...
	public def scrambleX(val f: DistArray[Complex]{(self.rank == 2)}) {
		...
		for (pl: Point{(self.rank == 1)} in Dist.makeUnique()) {
			...
			for (pt: Point{(self.rank == 2)} in (0..rLocal1) * (nxhh..nxhh)) {
			...
			for ([j1]: Point{(self.rank == 1)} in (1..(nxhh - 1))) {
				val j2 <: Int = (nxh - j1);
	...
	public def unscrambleX(val f: DistArray[Complex]{(self.rank == 2)}) {
		val nxh <: Int = (length2 / 2);
	...
	public def scrambleY(val f: DistArray[Complex]{(self.rank == 2)}) {

			for ([j1]: Point{(self.rank == 1)} in (1..(((length1 / 2)) - 1))) {
	...
	public def unscrambleY(val f: DistArray[Complex]{(self.rank == 2)}) {
	...
	public def transformRank(val f: DistArray[Complex]{(self.rank == 2)}, val rnk: Int, val inverse: Boolean) {
		...
		for (exponent: Point{(self.rank == 1)} in (0..(bits - 1))) {
			val stride <: Int = (1 << exponent(0));
			val width <: Int = (2 * stride);
			val km <: Int = ((((rnk == 0))?nyh:nxhh) / stride);
			val kmr <: Int = (km * r);
			for (pl: Point{(self.rank == 1)} in Dist.makeUnique()) 
				for ([k]: Point{(self.rank == 1)} in (0..(km - 1))) {
					val lb <: Int = (width * k);
					val mb <: Int = (lb + stride);
					for ([j]: Point{(self.rank == 1)} in (0..(stride - 1))) {
						val j1 <: Int = (j + lb);
						val j2 <: Int = (j + mb);
...
class LinearESOpenPIC2D {
	private static val NUMBER_OF_XBITS: Int = 5;
	private static val NUMBER_OF_YBITS: Int = 5;
	private static val XLENGTH: Int = (1 << NUMBER_OF_XBITS);
	private static val YLENGTH: Int = (1 << NUMBER_OF_YBITS);
	private static val PARTICLE_XDENSITY: Int = 128;
	private static val PARTICLE_YDENSITY: Int = 128;
	...
	private var particles: DistArray[Particle]{(self.rank == 1)};
	private var backgroundCharge: DistArray[Double]{(self.rank == 2)};
	...
	public def run() {
		...
		for (i: Point{(self.rank == 1)} in (0..(TIME_STEPS - 1))) {
			Console.OUT.println(("T = " + i(0)));
			val e <: DistArray[Double]{(self.rank == 1)} = stepTime(backgroundCharge);
			if (((i(0) % SORT_TIME) == 0)) {
{
					var time0: Long = System.currentTimeMillis();
					sortParticles(particles, backgroundCharge.dist);
					moveTime = (moveTime + ((System.currentTimeMillis() - time0)));
				}			}
			Console.OUT.println(((((("field, kinetic, total energies = " + e(0)) + ",") + e(1)) + ",") + e(2)));
			if ((i(0) == (TIME_STEPS - 1))) {
{
					val dRef <: Dist{(self.rank == 1)} = ((0..2)->here);
					val ref <: DistArray[Double]{(self.rank == 1)} = (DistArray.make[Double](dRef));
						...
						}					} else 					if (((((NUMBER_OF_XBITS == 3) && (NUMBER_OF_YBITS == 4)) && (PARTICLE_XDENSITY == 8)) && (PARTICLE_YDENSITY == 16))) {
{
							...
							Console.OUT.println(("final kinetic energy diff = " + (Math.abs((ref(1) - e(1))))));
							Console.OUT.println(("final final energy diff = " + (Math.abs((ref(2) - e(2))))));
						}					} else 					if (((((NUMBER_OF_XBITS == 5) && (NUMBER_OF_YBITS == 5)) && (PARTICLE_XDENSITY == 128)) && (PARTICLE_YDENSITY == 128))) {
{
							ref(0) = 14.29416386663976;
							ref(1) = 16556.89300233198082;
							ref(2) = 16571.18716619861880;
							Console.OUT.println(("final field energy diff = " + (Math.abs((ref(0) - e(0))))));
							Console.OUT.println(("final kinetic energy diff = " + (Math.abs((ref(1) - e(1))))));
							Console.OUT.println(("final final energy diff = " + (Math.abs((ref(2) - e(2))))));
						}					} else 					if (((((NUMBER_OF_XBITS == 6) && (NUMBER_OF_YBITS == 7)) && (PARTICLE_XDENSITY == 384)) && (PARTICLE_YDENSITY == 768))) {
{
							ref(0) = 135.15662890071584;
							ref(1) = 295436.86221817752812;
							ref(2) = 295572.01884707825957;
							Console.OUT.println(("final field energy diff = " + (Math.abs((ref(0) - e(0))))));
							Console.OUT.println(("final kinetic energy diff = " + (Math.abs((ref(1) - e(1))))));
							Console.OUT.println(("final final energy diff = " + (Math.abs((ref(2) - e(2))))));
						}					} else 
	...
	public def initialize() {
		...
		val position <: DistArray[Double]{(self.rank == 2)} = genParticlePosition(NPARTICLES);
		Console.OUT.println("Init: particle velocity");
		val velocity <: DistArray[Double]{(self.rank == 2)} = genParticleVelocity(NPARTICLES);
		...
		val info <: DistArray[Int]{(self.rank == 1)} = makeInfoArray();
		particles = moveParticles(particles, dField, info);
		Console.OUT.println("Init: background charge");
		backgroundCharge = (DistArray.make[Double](dField));
		val backgroundGuards <: DistArray[Double]{(self.rank == 2)} = makeGuards(dField);
	...
	public def genParticlePosition(val np: Int): DistArray[Double]{(self.rank == 2)} {
		val X <: Int = 0;
		val Y <: Int = 1;
		...
		val BIG <: Double = 0.5;
		val dResult <: Dist{(self.rank == 2)} = X10Util.distBlockStar2((0..(np - 1)), (0..1));
		val result <: DistArray[Double]{(self.rank == 2)} = (DistArray.make[Double](dResult));
		...
		if ((totalParticles != NPARTICLES)) {
{
		...
		for (pl: Point{(self.rank == 1)} in Dist.makeUnique()) {
			...
			for (k: Point{(self.rank == 1)} in (0..(kc - 1))) {
					...
					if ((i >= MAX_ITER)) {
{
			...
			for (j: Point{(self.rank == 1)} in (0..(jc - 1))) {
					...
					if ((i >= MAX_ITER)) {
{
			...
			kc++;
			for ([n]: Point{(self.rank == 1)} in (0..(X10Util.minInt(particlesPerPlace, PARTICLE_XDENSITY) - 1))) {
				var nn: Int = (n + koff);
					...
					if ((i >= MAX_ITER)) {
{
			...
			for ([n]: Point{(self.rank == 1)} in (0..(particlesPerPlace - 1))) {
				var nn: Int = (n + koff);
							...
							if ((i >= MAX_ITER)) {
{
	...
	public def genParticleVelocity(val n: Int): DistArray[Double]{(self.rank == 2)} {
		val X <: Int = 0;
		val Y <: Int = 1;
		val dResult <: Dist{(self.rank == 2)} = X10Util.distBlockStar2((0..(n - 1)), (0..1));
		val result <: DistArray[Double]{(self.rank == 2)} = (DistArray.make[Double](dResult));

			for ([i]: Point{(self.rank == 1)} in (0..(PARTICLE_YDENSITY - 1))) {
				val offset <: Int = (PARTICLE_XDENSITY * i);
				for ([j]: Point{(self.rank == 1)} in (0..(PARTICLE_XDENSITY - 1))) {
					...
					val vy <: Double = (THERMAL_YVELOCITY * r.nextNormal());
{
		...
		val dUnique <: Dist{(self.rank == 1)} = (Dist.makeUnique() as Dist{(self.rank == 1)});
		val numParticles <: DistArray[Int]{(self.rank == 1)} = (DistArray.make[Int](dUnique, (pt:Point{(self.rank == 1)}) => {
			return 0;
		}));
		val sumX <: DistArray[Double]{(self.rank == 1)} = (DistArray.make[Double](dUnique, (pt:Point{(self.rank == 1)}) => {
			return 0.0;
		}));
		val sumY <: DistArray[Double]{(self.rank == 1)} = (DistArray.make[Double](dUnique, (pt:Point{(self.rank == 1)}) => {
			return 0.0;
		}));
		for (pl: Point{(self.rank == 1)} in Dist.makeUnique()) {
			val rLocal1 <: Region{(self.rank == 1)} = X10Util.getLRank2D(dResult, 0);
			for (pt: Point{(self.rank == 2)} in (0..rLocal1) * (X..X)) {
				sumX(pl) = (sumX(pl) + result(pt));
				numParticles(pl) = (numParticles(pl) + 1);
			}
			for (pt: Point{(self.rank == 2)} in (0..rLocal1) * (Y..Y)) 
	...
	public def genParticles(val n: Int, val position: DistArray[Double]{(self.rank == 2)}, val velocity: DistArray[Double]{(self.rank == 2)}): DistArray[Particle]{(self.rank == 1)} {
		val dResult <: Dist{(self.rank == 1)} = (Dist.makeBlock((0..(n - 1))) as Dist{(self.rank == 1)});
		return (DistArray.make[Particle](dResult, ([i]:Point{(self.rank == 1)}) => {
			return new Particle(position(i, 0), position(i, 1), velocity(i, 0), velocity(i, 1));
		}));
	}

	public def genInitialDist(val particles: DistArray[Particle]{(self.rank == 1)}): Dist{(self.rank == 2)} {
		val MAX_PLACES <: Int = Place.MAX_PLACES;
		val edgely <: Double = ((BOUNDARY_CONDITION == 2))?1.0:0.0;
		...
		val dUnique <: Dist{(self.rank == 1)} = (Dist.makeUnique() as Dist{(self.rank == 1)});
		val yRegions <: DistArray[Region{(self.rank == 1)}]{(self.rank == 1)} = ((DistArray.make[Region{(self.rank == 1)}](dUnique)) as DistArray[Region{(self.rank == 1)}]{(self.rank == 1)});
		for (pl: Point{(self.rank == 1)} in Dist.makeUnique()) {
			...
			if (here.isLast()) {
					...
					ub = y;
			}			yRegions(pl) = ((((lb + 0.5)) as Int)..(((((ub + 0.5)) as Int) - 1)));
		}
		val pHere <: Place = here;
		val dPlace <: Dist{(self.rank == 1)} = ((((0..(MAX_PLACES - 1))->here)) as Dist{(self.rank == 1)});
		val rBuffer <: DistArray[Region{(self.rank == 1)}]{(self.rank == 1)} = ((DistArray.make[Region{(self.rank == 1)}](dPlace)) as DistArray[Region{(self.rank == 1)}]{(self.rank == 1)});
		for (pl: Point{(self.rank == 1)} in Dist.makeUnique()) {
		...
		for (pl: Point{(self.rank == 1)} in Dist.makeUnique()) {
			...
			val dLocal <: Dist{(self.rank == 2)} = ((((0..ry) * (0..XLENGTH)->p)) as Dist{(self.rank == 2)});
	...
	public def stepTime(val ionBackground: DistArray[Double]{(self.rank == 2)}): DistArray[Double]{(self.rank == 1)} {
		var time1: Long = System.currentTimeMillis();
		var time2: Long = 0;
		val CHARGE <: Double = ELECTRON_CHARGE;
		val dIonBG <: Dist{(self.rank == 2)} = ionBackground.dist;
		val chargeDensityGuards <: DistArray[Double]{(self.rank == 2)} = makeGuards(dIonBG);
		val chargeDensity <: DistArray[Double]{(self.rank == 2)} = ((DistArray.make[Double](dIonBG, (pt:Point{(self.rank == 2)}) => {
			return 0.0;
		})) as DistArray[Double]{(self.rank == 2)});
		depositCharge(particles, chargeDensity, chargeDensityGuards, CHARGE);
		for (pt: Point{(self.rank == 2)} in dIonBG) 
		...
		val uniformChargeDensity <: DistArray[Double]{(self.rank == 2)} = moveFieldToUniform(chargeDensity);
		val complexChargeDensity <: DistArray[Complex]{(self.rank == 2)} = ArrayUtil.doubleToComplex(doubleDimensions(uniformChargeDensity));
		time1 = System.currentTimeMillis();
		redistTime = (redistTime + ((time1 - time2)));
		val complexForceX <: DistArray[Complex]{(self.rank == 2)} = solver.makeForceArray();
		val complexForceY <: DistArray[Complex]{(self.rank == 2)} = solver.makeForceArray();
		...
		val gx <: DistArray[Double]{(self.rank == 2)} = (DistArray.make[Double](dChargeDensityGuards));
		val gy <: DistArray[Double]{(self.rank == 2)} = (DistArray.make[Double](dChargeDensityGuards));
		val forceX <: DistArray[Double]{(self.rank == 2)} = cropField(ArrayUtil.complexToDouble(complexForceX), gx);
		val forceY <: DistArray[Double]{(self.rank == 2)} = cropField(ArrayUtil.complexToDouble(complexForceY), gy);
		val fcx <: DistArray[Double]{(self.rank == 2)} = moveFieldToDist(forceX, dOld);
		val fcy <: DistArray[Double]{(self.rank == 2)} = moveFieldToDist(forceY, dOld);
		loadGuards(fcx, gx);
		loadGuards(fcy, gy);
		time1 = System.currentTimeMillis();
		redistTime = (redistTime + ((time1 - time2)));
...
{
				val count <: DistArray[Int]{(self.rank == 1)} = countParticles(particles, dOld);
		...
		val dResult <: Dist{(self.rank == 1)} = ((((0..2)->here)) as Dist{(self.rank == 1)});
		val result <: DistArray[Double]{(self.rank == 1)} = (DistArray.make[Double](dResult));
		result(0) = we;
		result(1) = wk;
		result(2) = (we + wk);
		time1 = System.currentTimeMillis();
	...
	public def makeGuards(val dData: Dist{(self.rank == 2)}): DistArray[Double]{(self.rank == 2)} {
		var dResult: Dist{(self.rank == 2)} = (((Region.makeEmpty(2)->here)) as Dist{(self.rank == 2)});
		for (pl: Point{(self.rank == 1)} in Dist.makeUnique()) {
			val p <: Place = Dist.makeUnique()(pl);
			...
			dResult = (dResult || ((((ub + 1)..(ub + 1)) * (0..rLocal2)->p)));
		}
		return (DistArray.make[Double](dResult, (pt:Point{(self.rank == 2)}) => {
			return 0.0;
		}));
	}

	public def depositCharge(val particles: DistArray[Particle]{(self.rank == 1)}, val fieldCharge: DistArray[Double]{(self.rank == 2)}, val guards: DistArray[Double]{(self.rank == 2)}, val charge: Double) {
		for (pl: Point{(self.rank == 1)} in Dist.makeUnique()) {
			...
			val localCharge <: DistArray[Double]{(self.rank == 2)} = (DistArray.make[Double](dLocal, (pt:Point{(self.rank == 2)}) => {
				return 0.0;
			}));
...
{
				for (pt: Point{(self.rank == 2)} in (fieldCharge.dist | here)) 
					fieldCharge(pt) = localCharge(pt);
				for (pt: Point{(self.rank == 2)} in (guards.dist | here)) 
					guards(pt) = localCharge(pt);
			}		}
	}

	public def addGuards(val data: DistArray[Double]{(self.rank == 2)}, val guards: DistArray[Double]{(self.rank == 2)}) {
		...
		for (pl: Point{(self.rank == 1)} in Dist.makeUnique()) {
			...
			if (!here.isLast()) {
{
					val sBuffer <: DistArray[Double]{(self.rank == 2)} = ((DistArray.make[Double](rLocalG-> here, (pt:Point{(self.rank == 2)}) => {
						return guards(pt);
					})) as DistArray[Double]{(self.rank == 2)});
{
						val dData <: Dist{(self.rank == 2)} = data.dist;
						val rLocalD <: Region{(self.rank == 2)} = X10Util.get2DLRegion(dData);
						for (pt: Point{(self.rank == 2)} in ((rLocalD && rLocalG))) 
							data(pt) = (data(pt) + sBuffer(pt));
					}				}			}
			for (pt: Point{(self.rank == 2)} in rLocalG) 
				guards(pt) = 0.0;
		}
	}

	public def doubleDimensions(val data: DistArray[Double]{(self.rank == 2)}): DistArray[Double]{(self.rank == 2)} {
		...
		val dResult <: Dist{(self.rank == 2)} = X10Util.distBlockStar2((0..((2 * rData1.size()) - 1)), (0..((2 * rData2.size()) - 1)));
		val result <: DistArray[Double]{(self.rank == 2)} = (DistArray.make[Double](dResult, (pt:Point{(self.rank == 2)}) => {
			return 0.0;
		}));
		for (pl: Point{(self.rank == 1)} in Dist.makeUnique()) {
			val pDst <: Place = Place.place((pl(0) / 2));
			val dData <: Dist{(self.rank == 2)} = data.dist;
			val rLocal <: Region{(self.rank == 2)} = X10Util.get2DLRegion(dData);
			val sBuffer <: DistArray[Double]{(self.rank == 2)} = ((DistArray.make[Double](rLocal-> here, (pt:Point{(self.rank == 2)}) => {
				return data(pt);
			})) as DistArray[Double]{(self.rank == 2)});

				for (pt: Point{(self.rank == 2)} in rLocal) 
					result(pt) = sBuffer(pt);
		}
		return result;
	}

	public def moveFieldToUniform(val data: DistArray[Double]{(self.rank == 2)}): DistArray[Double]{(self.rank == 2)} {
		val rData <: Region{(self.rank == 2)} = data.region;
		return moveFieldToDist(data, X10Util.distBlockStar1(rData));
	}

	public def moveFieldToDist(val data: DistArray[Double]{(self.rank == 2)}, val dDst: Dist{(self.rank == 2)}): DistArray[Double]{(self.rank == 2)} {
		...
		val rRank2 <: Region{(self.rank == 1)} = (0..(rRank2t.size() - 1));
		val rRows <: Region{(self.rank == 1)} = (0..(MAX_ROWS - 1));
		val dBuffer <: Dist{(self.rank == 3)} = X10Util.extendDistRank3D(X10Util.extendDistRank2D(dUnique, rRows), rRank2);
		val srcBuffer <: DistArray[Double]{(self.rank == 3)} = (DistArray.make[Double](dBuffer, (pt:Point{(self.rank == 3)}) => {
			return 0.0;
		}));
		val dstBuffer <: DistArray[Double]{(self.rank == 3)} = (DistArray.make[Double](dBuffer, (pt:Point{(self.rank == 3)}) => {
			return 0.0;
		}));
		val offsets <: DistArray[Int]{(self.rank == 1)} = (DistArray.make[Int](dUnique, (pt:Point{(self.rank == 1)}) => {
			return 0;
		}));
		val sizes <: DistArray[Int]{(self.rank == 1)} = (DistArray.make[Int](dUnique, (pt:Point{(self.rank == 1)}) => {
			return 0;
		}));
		val inLt <: DistArray[Int]{(self.rank == 1)} = (DistArray.make[Int](dUnique, (pt:Point{(self.rank == 1)}) => {
			return 0;
		}));
		val inRt <: DistArray[Int]{(self.rank == 1)} = (DistArray.make[Int](dUnique, (pt:Point{(self.rank == 1)}) => {
			return 0;
		}));
		val outLt <: DistArray[Int]{(self.rank == 1)} = (DistArray.make[Int](dUnique, (pt:Point{(self.rank == 1)}) => {
			return 0;
		}));
		val outRt <: DistArray[Int]{(self.rank == 1)} = (DistArray.make[Int](dUnique, (pt:Point{(self.rank == 1)}) => {
			return 0;
		}));
		for ([pl]: Point{(self.rank == 1)} in Dist.makeUnique()) {
			val dData <: Dist{(self.rank == 2)} = data.dist;
			...
			for ([i, j]: Point{(self.rank == 2)} in rLocal) 
		...
		do {
			for (pl: Point{(self.rank == 1)} in Dist.makeUnique()) {
				...
				for ([i1]: Point{(self.rank == 1)} in (0..(sizes(pl) - 1))) {
			...
			iter++;
			val pr <: DistArray[Int]{(self.rank == 1)} = (DistArray.make[Int](dUnique, (pt:Point{(self.rank == 1)}) => {
				return 0;
			}));
			val ter <: DistArray[Int]{(self.rank == 1)} = (DistArray.make[Int](dUnique, (pt:Point{(self.rank == 1)}) => {
				return 0;
			}));
			for (pl: Point{(self.rank == 1)} in Dist.makeUnique()) {
				inLt(pl) = 0;
				...
				if ((outRt(pl) > 0)) {
{
						...
						val rBuffer <: Region{(self.rank == 2)} = (0..(bsize - 1)) * (0..rRank2);
						val sBuffer <: DistArray[Double]{(self.rank == 2)} = ((DistArray.make[Double](rBuffer-> here, ([i, j]:Point{(self.rank == 2)}) => {
							return srcBuffer(pl, ((i + sizes(pl)) - outRt(pl)), j);
						})) as DistArray[Double]{(self.rank == 2)});
{
							for ([i, j]: Point{(self.rank == 2)} in rBuffer) 
				...
				if ((outRt(pl) > 0)) {
{
						...
						for ([i, j]: Point{(self.rank == 2)} in (0..(rsize - 1)) * (0..rRank2)) 
							dstBuffer(pl, ((MAX_ROWS - rsize) + i), j) = srcBuffer(pl, (sizes(pl) + i), j);
					}				}
				if (((sizes(pl) > 0) && (inLt(pl) > 0))) {

						for ([i1]: Point{(self.rank == 1)} in (1..sizes(pl))) {
							val i2 <: Int = (sizes(pl) - i1);
							for ([j]: Point{(self.rank == 1)} in rRank2) 
								srcBuffer(pl, (i2 + inLt(pl)), j) = srcBuffer(pl, i2, j);
						}
				}				for (pt: Point{(self.rank == 3)} in (pl..pl) * (0..(inLt(pl) - 1)) * (0..rRank2)) 
						...
						})) as DistArray[Double]{(self.rank == 2)});
{
			...
			for ([pl]: Point{(self.rank == 1)} in Dist.makeUnique()) {
				...
				if (((sizes(pl) > 0) && (outLt(pl) > 0))) {
						...
						for ([i]: Point{(self.rank == 1)} in (0..(sizes(pl) - 1))) 
							for ([j]: Point{(self.rank == 1)} in rRank2) 
...
;
				}				for ([i, j]: Point{(self.rank == 2)} in (0..(inRt(pl) - 1)) * (0..rRank2)) 
					...
					if ((COMPILER_INSERTED_LIB.sum(pr) != 0)) {
{
			...
			}			val dFlags <: Dist{(self.rank == 1)} = ((0..1)->here);
			val flags <: DistArray[Int]{(self.rank == 1)} = (DistArray.make[Int](dFlags));
			...
			if ((flags(0) != 0)) {
{
		...
		val result <: DistArray[Double]{(self.rank == 2)} = (DistArray.make[Double](dDst));
		for ([pl]: Point{(self.rank == 1)} in Dist.makeUnique()) {
			val rLocal1 <: Region{(self.rank == 1)} = X10Util.getLRank2D(dDst, 0);
			if (X10Util.ENABLE_CHECKS) {
{
			...
			for ([i, j]: Point{(self.rank == 2)} in (0..(sizes(pl) - 1)) * (0..rRank2)) 
				result((offsets(pl) + i), j) = srcBuffer(pl, i, j);
		}
		return result;
	}

	public def cropField(val data: DistArray[Double]{(self.rank == 2)}, val guards: DistArray[Double]{(self.rank == 2)}): DistArray[Double]{(self.rank == 2)} {
		val rRank1 <: Region{(self.rank == 1)} = (0..(YLENGTH - 1));
		val rRank2 <: Region{(self.rank == 1)} = (0..XLENGTH);
		val dGuard <: Dist{(self.rank == 2)} = guards.dist;
		val rGuard <: Region{(self.rank == 2)} = X10Util.get2DPRegion(dGuard, Place.place((Place.MAX_PLACES - 1)));
...
{
					val pDst1 <: Place = Place.place((2 * pl(0)));
					val pDst2 <: Place = Place.place(((2 * pl(0)) + 1));
					val sBuffer <: DistArray[Double]{(self.rank == 2)} = ((DistArray.make[Double](rLocalS-> here, (pt:Point{(self.rank == 2)}) => {
						return data(pt);
					})) as DistArray[Double]{(self.rank == 2)});
{
						val rLocalD <: Region{(self.rank == 2)} = X10Util.get2DLRegion(dResult);
						for (pt: Point{(self.rank == 2)} in rLocalD) 
							result(pt) = sBuffer(pt);
					}{
						val rLocalD <: Region{(self.rank == 2)} = X10Util.get2DLRegion(dResult);
						for (pt: Point{(self.rank == 2)} in rLocalD) 
...
{
					val sBuffer <: DistArray[Double]{(self.rank == 2)} = ((DistArray.make[Double](rGuard-> here, (pt:Point{(self.rank == 2)}) => {
						return data(pt);
					})) as DistArray[Double]{(self.rank == 2)});

						for (pt: Point{(self.rank == 2)} in rGuard) 
	...
	public def loadGuards(val data: DistArray[Double]{(self.rank == 2)}, val guards: DistArray[Double]{(self.rank == 2)}) {
		for (pl: Point{(self.rank == 1)} in Dist.makeUnique()) 
			if (!here.isFirst()) {
{
					val dGuard <: Dist{(self.rank == 2)} = guards.dist;
	...
	public def makeInfoArray(): DistArray[Int]{(self.rank == 1)} {
		val dInfo <: Dist{(self.rank == 1)} = ((((0..6)->here)) as Dist{(self.rank == 1)});
		return (DistArray.make[Int](dInfo, (pt:Point{(self.rank == 1)}) => {
			return 0;
		}));
	}

	public def moveParticles(val particles: DistArray[Particle]{(self.rank == 1)}, val dFieldSpace: Dist{(self.rank == 2)}, val info: DistArray[Int]{(self.rank == 1)}): DistArray[Particle]{(self.rank == 1)} {
		val MAX <: Int = MOVE_PARTICLES_MAX;
		val MAX_BUFFER <: Int = MOVE_PARTICLES_MAX_BUFFER;
		val ITER_MAX <: Int = 20;
		val rPart <: Region{(self.rank == 1)} = (0..(MAX - 1));
		val rBuffer <: Region{(self.rank == 1)} = (0..(MAX_BUFFER - 1));
		val dUnique <: Dist{(self.rank == 1)} = (Dist.makeUnique() as Dist{(self.rank == 1)});
		...
		val part <: DistArray[Particle]{(self.rank == 2)} = (DistArray.make[Particle](dPart));
		val sBufferL <: DistArray[Particle]{(self.rank == 2)} = (DistArray.make[Particle](dBuffer));
		val sBufferR <: DistArray[Particle]{(self.rank == 2)} = (DistArray.make[Particle](dBuffer));
		val rBufferL <: DistArray[Particle]{(self.rank == 2)} = (DistArray.make[Particle](dBuffer));
		val rBufferR <: DistArray[Particle]{(self.rank == 2)} = (DistArray.make[Particle](dBuffer));
		val inLt <: DistArray[Int]{(self.rank == 1) && (self.dist == dUnique)} = (DistArray.make[Int](dUnique, (pt:Point{(self.rank == 1)}) => {
			return 0;
		}));
		val inRt <: DistArray[Int]{(self.rank == 1) && (self.dist == dUnique)} = (DistArray.make[Int](dUnique, (pt:Point{(self.rank == 1)}) => {
			return 0;
		}));
		val outLt <: DistArray[Int]{(self.rank == 1) && (self.dist == dUnique)} = (DistArray.make[Int](dUnique, (pt:Point{(self.rank == 1)}) => {
			return 0;
		}));
		val outRt <: DistArray[Int]{(self.rank == 1) && (self.dist == dUnique)} = (DistArray.make[Int](dUnique, (pt:Point{(self.rank == 1)}) => {
			return 0;
		}));
		val nPart <: DistArray[Int]{(self.rank == 1) && (self.dist == dUnique)} = (DistArray.make[Int](dUnique, (pt:Point{(self.rank == 1)}) => {
			return 0;
		}));
		var nPartOld: DistArray[Int]{(self.rank == 1) && (self.dist == dUnique)} = (DistArray.make[Int](dUnique, (pt:Point{(self.rank == 1)}) => {
			return 0;
		}));
		val holes <: DistArray[Int]{(self.rank == 2)} = (DistArray.make[Int](dPart, (pt:Point{(self.rank == 2)}) => {
			return 0;
		}));
		val dFlags <: Dist{(self.rank == 1)} = ((0..3)->here);
		val nSent <: DistArray[Int]{(self.rank == 1) && (self.dist == dUnique)} = (DistArray.make[Int](dUnique, (pt:Point{(self.rank == 1)}) => {
			return 0;
		}));
		val ibflg <: DistArray[Int]{(self.rank == 1)} = (DistArray.make[Int](dFlags, (pt:Point{(self.rank == 1)}) => {
			return 0;
		}));
		for ([pl]: Point{(self.rank == 1)} in Dist.makeUnique()) {
...
{
					val offset <: Int = rLocal.min(0);
					for ([i]: Point{(self.rank == 1)} in rLocal) 
		...
		var iter: Int = 2;
		var nter: Int = 0;
		for (pt: Point{(self.rank == 1)} in (0..6)) 
			info(pt) = 0;
		...
		do {
			var mter: Int = 0;
			val overflow <: DistArray[Int]{(self.rank == 1) && (self.dist == dUnique)} = (DistArray.make[Int](dUnique, (pt:Point{(self.rank == 1)}) => {
				return 0;
			}));
			for ([pl]: Point{(self.rank == 1)} in Dist.makeUnique()) {
				...
				outLt(pl) = 0;
				outRt(pl) = 0;
				for ([i]: Point{(self.rank == 1)} in (0..(nPart(pl) - 1))) {
							...
							if ((outRt(pl) < rBuffer.size())) {
{
			...
			do {
				...
				val outgoing <: DistArray[Int]{(self.rank == 1)} = (DistArray.make[Int](dUnique));
				for ([pl]: Point{(self.rank == 1)} in Dist.makeUnique()) {
					val rLocal <: Region{(self.rank == 2)} = (((dFieldSpace | here)).region as Region{(self.rank == 2)});
					val rLocal1 <: Region{(self.rank == 1)} = X10Util.getRank2D(rLocal, 0);
					val ub <: Double = (rLocal1.max(0) + 1);
					...
					outRt(pl) = 0;
					var belowLeft: Int = 0;
					for ([i]: Point{(self.rank == 1)} in (0..(inRt(pl) - 1))) {
					...
					}					for ([i]: Point{(self.rank == 1)} in (0..(inLt(pl) - 1))) {
										...
										if ((outLt(pl) < rBuffer.size())) {
{
				...
				val tMaxSize <: DistArray[Int]{(self.rank == 1) && (self.dist == dUnique)} = ((((nPart + inLt) + inRt) - nSent));
				val tMinSize <: DistArray[Int]{(self.rank == 1) && (self.dist == dUnique)} = (((DistArray.make[Int](dUnique, (pt:Point{(self.rank == 1)}) => {
					return rSize;
				}))) - tMaxSize);
				...
				if ((err > 0)) {
{
				...
				for ([pl]: Point{(self.rank == 1)} in Dist.makeUnique()) {
					...
					for ([i]: Point{(self.rank == 1)} in (0..(nMove - 1))) 
		...
		if ((info(5) != info(6))) {
{
	...
	private def transferParticles(val leftOutBuffer: DistArray[Particle]{(self.rank == 2)}, val rightOutBuffer: DistArray[Particle]{(self.rank == 2)}, val outLeft: DistArray[Int]{(self.rank == 1)}, val outRight: DistArray[Int]{(self.rank == 1)}, val leftInBuffer: DistArray[Particle]{(self.rank == 2)}, val rightInBuffer: DistArray[Particle]{(self.rank == 2)}, val inLeft: DistArray[Int]{(self.rank == 1)}, val inRight: DistArray[Int]{(self.rank == 1)}) {
		for ([pl]: Point{(self.rank == 1)} in Dist.makeUnique()) {
			val pLeft <: Place = here.prev();
			val pRight <: Place = here.next();
			val rLeft <: Region{(self.rank == 1)} = (0..(outLeft(pl) - 1));
			val rRight <: Region{(self.rank == 1)} = (0..(outRight(pl) - 1));
			val lBuffer <: DistArray[Particle]{(self.rank == 1)} = ((DistArray.make[Particle](rLeft-> here, ([i]:Point{(self.rank == 1)}) => {
				return leftOutBuffer(pl, i);
			})) as DistArray[Particle]{(self.rank == 1)});
			val rBuffer <: DistArray[Particle]{(self.rank == 1)} = ((DistArray.make[Particle](rRight-> here, ([i]:Point{(self.rank == 1)}) => {
				return rightOutBuffer(pl, i);
			})) as DistArray[Particle]{(self.rank == 1)});
{
				val pid <: Int = here.id;
				inRight(pid) = lBuffer.region.size();
				for ([i]: Point{(self.rank == 1)} in lBuffer) 
					rightInBuffer(pid, i) = lBuffer(i);
			}{
				val pid <: Int = here.id;
				inLeft(pid) = rBuffer.region.size();
				for ([i]: Point{(self.rank == 1)} in rBuffer) 
					leftInBuffer(pid, i) = rBuffer(i);
			}		}
	}

	private def packParticleArray(val part: DistArray[Particle]{(self.rank == 2)}, val nParticles: DistArray[Int]{(self.rank == 1)}): DistArray[Particle]{(self.rank == 1)} {
		var lb: Int = 0;
		var dResult: Dist{(self.rank == 1)} = (((Region.makeEmpty(1)->here)) as Dist{(self.rank == 1)});
		for (pt: Point{(self.rank == 1)} in Dist.makeUnique()) {
			var rsize: Int = X10Util.pullInt(nParticles, pt);
			var p: Place = Place.place(pt(0));
			dResult = (dResult || (((lb..((lb + rsize) - 1))->p)));
			lb = (lb + rsize);
		}
		val result <: DistArray[Particle]{(self.rank == 1)} = (DistArray.make[Particle](dResult));
		for ([pl]: Point{(self.rank == 1)} in Dist.makeUnique()) {
			...
			if ((rLocal.size() > 0)) {
{
	...
	public def pushParticles(val particles: DistArray[Particle]{(self.rank == 1)}, val forceChargeX: DistArray[Double]{(self.rank == 2)}, val fcGuardX: DistArray[Double]{(self.rank == 2)}, val forceChargeY: DistArray[Double]{(self.rank == 2)}, val fcGuardY: DistArray[Double]{(self.rank == 2)}): Double {
		var lx: Double = 0.0;
		var ly: Double = 0.0;
		var rx: Double = 0.0;
		var ry: Double = 0.0;
		...
		val energy <: DistArray[Double]{(self.rank == 1)} = (DistArray.make[Double](dEnergy));
		for (pl: Point{(self.rank == 1)} in Dist.makeUnique()) {
			val fcLocalX <: DistArray[Double]{(self.rank == 2)} = makeLocalSpace(forceChargeX, fcGuardX);
			val fcLocalY <: DistArray[Double]{(self.rank == 2)} = makeLocalSpace(forceChargeY, fcGuardY);
			for (pt: Point{(self.rank == 1)} in (particles.dist | here)) 
	...
	private def makeLocalSpace(val data: DistArray[Double]{(self.rank == 2)}, val guards: DistArray[Double]{(self.rank == 2)}): DistArray[Double]{(self.rank == 2)} {
		val dData <: Dist{(self.rank == 2)} = data.dist;
		val dGuard <: Dist{(self.rank == 2)} = guards.dist;
		val rLocalD <: Region{(self.rank == 2)} = X10Util.get2DLRegion(dData);
		...
		val result <: DistArray[Double]{(self.rank == 2)} = (DistArray.make[Double](dResult));
{
			for (pt: Point{(self.rank == 2)} in (data.dist | here)) 
				result(pt) = data(pt);
			for (pt: Point{(self.rank == 2)} in (guards.dist | here)) 
				result(pt) = guards(pt);
		}		return result;
	...
	private def pushParticle(val particles: DistArray[Particle]{(self.rank == 1)}, val pt: Point{(self.rank == 1)}, val forceChargeX: DistArray[Double]{(self.rank == 2)}, val forceChargeY: DistArray[Double]{(self.rank == 2)}, val quantum: Double, val edgelx: Double, val edgely: Double, val edgerx: Double, val edgery: Double, val energy: DistArray[Double]{(self.rank == 1)}) {
		val p <: Particle = particles(pt);
		val x <: Int = (p.getX() as Int);
		val y <: Int = (p.getY() as Int);
		val dx <: Double = (p.getX() - x);
				...
				if (((nx < edgelx) || (nx >= edgerx))) {
{
	...
	public def countParticles(val particles: DistArray[Particle]{(self.rank == 1)}, val dField: Dist{(self.rank == 2)}): DistArray[Int]{(self.rank == 1)} {
		val dCount <: Dist{(self.rank == 1)} = X10Util.get2DDistRank(dField, 0);
		val count <: DistArray[Int]{(self.rank == 1)} = (DistArray.make[Int](dCount, (pt:Point{(self.rank == 1)}) => {
			return 0;
		}));
		for (pl: Point{(self.rank == 1)} in Dist.makeUnique()) 
			for (pt: Point{(self.rank == 1)} in (particles.dist | here)) 
	...
	public def repartition(val particleCount: DistArray[Int]{(self.rank == 1)}, val dField: Dist{(self.rank == 2)}, val countAve: Double): Dist{(self.rank == 2)} {
		val MAX <: Int = 1024;
		val dUnique <: Dist{(self.rank == 1)} = (Dist.makeUnique() as Dist{(self.rank == 1)});
		...
		val border <: DistArray[Double]{(self.rank == 1)} = (DistArray.make[Double](dUnique, (pt:Point{(self.rank == 1)}) => {
			return 0.0;
		}));
		val particleTotals <: DistArray[Int]{(self.rank == 1)} = (DistArray.make[Int](dUnique, (pt:Point{(self.rank == 1)}) => {
			return 0;
		}));
		val gridTotals <: DistArray[Int]{(self.rank == 1)} = (DistArray.make[Int](dUnique, (pt:Point{(self.rank == 1)}) => {
			return 0;
		}));
		val dBuffer <: Dist{(self.rank == 2)} = X10Util.extendDistRank2D((Dist.makeUnique() as Dist{(self.rank == 1)}), (0..(MAX - 1)));
		val sCountBuffer <: DistArray[Int]{(self.rank == 2)} = (DistArray.make[Int](dBuffer));
		val rCountBuffer <: DistArray[Int]{(self.rank == 2)} = (DistArray.make[Int](dBuffer));
		val sendCount <: DistArray[Int]{(self.rank == 1)} = (DistArray.make[Int](dUnique));
		val sendGrids <: DistArray[Int]{(self.rank == 1)} = (DistArray.make[Int](dUnique));
		val recvCount <: DistArray[Int]{(self.rank == 1)} = (DistArray.make[Int](dUnique));
		val recvGrids <: DistArray[Int]{(self.rank == 1)} = (DistArray.make[Int](dUnique));
		val finished <: DistArray[Int]{(self.rank == 1)} = (DistArray.make[Int](dUnique, (pt:Point{(self.rank == 1)}) => {
			return 0;
		}));
		for ([pl]: Point{(self.rank == 1)} in Dist.makeUnique()) {
		...
		val runningPTotals <: DistArray[Int]{(self.rank == 1)} = parallelPrefix(particleTotals);
		val runningGTotals <: DistArray[Int]{(self.rank == 1)} = parallelPrefix(gridTotals);
		for (pl: Point{(self.rank == 1)} in Dist.makeUnique()) {
		...
		do {
			for ([pl]: Point{(self.rank == 1)} in Dist.makeUnique()) {
				...
				if ((surplus > 0.0)) {
{
						val dSBuffer <: Region{(self.rank == 1)} = (0..(sendGrids(pl) - 1));
						val sBuffer <: DistArray[Int]{(self.rank == 1)} = ((DistArray.make[Int](dSBuffer-> here, ([i]:Point{(self.rank == 1)}) => {
						...
						val nGrids <: Int = sendGrids(pl);
{
							...
							for ([i]: Point{(self.rank == 1)} in dSBuffer) 
				...
				}				if (!here.isFirst()) {
{
						...
						for (pt: Point{(self.rank == 2)} in (pl..pl) * (0..(gsize - 1))) 
				...
				}				if (!here.isLast()) {
{
		...
		val pHere <: Place = here;
		val dBounds <: Dist{(self.rank == 1)} = ((0..Place.MAX_PLACES)->here);
		val lowerbounds <: DistArray[Int]{(self.rank == 1)} = (DistArray.make[Int](dBounds));
		val upperbounds <: DistArray[Int]{(self.rank == 1)} = (DistArray.make[Int](dBounds));
		for (pl: Point{(self.rank == 1)} in Dist.makeUnique()) {
			val edge <: Double = border(pl);

				lowerbounds(pl) = (((edge + 0.5)) as Int);
		}
		for (i: Point{(self.rank == 1)} in (0..(Place.MAX_PLACES - 1))) {
			val lb <: Int = lowerbounds(i);
			for (j: Point{(self.rank == 1)} in ((i(0) + 1)..(Place.MAX_PLACES - 1))) 
				if ((lowerbounds(j) <= lb)) {

						lowerbounds(j) = (lb + 1);
				}		}
		for (i: Point{(self.rank == 1)} in (0..(Place.MAX_PLACES - 2))) 
			upperbounds(i) = (lowerbounds((i + Point.make([1]))) - 1);
		upperbounds((Place.MAX_PLACES - 1)) = (YLENGTH - 1);
		var dResult: Dist{(self.rank == 2)} = (((Region.makeEmpty(2)->here)) as Dist{(self.rank == 2)});
		for (pl: Point{(self.rank == 1)} in Dist.makeUnique()) {
			val p <: Place = Dist.makeUnique()(pl);
			dResult = (dResult || (((lowerbounds(pl)..upperbounds(pl)) * (0..rRows)->p)));
		}
		return dResult;
	}

	public def sortParticles(val particles: DistArray[Particle]{(self.rank == 1)}, val dField: Dist{(self.rank == 2)}): DistArray[Particle]{(self.rank == 1)} {
		val dParticles <: Dist{(self.rank == 1)} = particles.dist;
		val result <: DistArray[Particle]{(self.rank == 1)} = (DistArray.make[Particle](dParticles));
		val indices <: DistArray[Int]{(self.rank == 1)} = (DistArray.make[Int](dParticles));
		val dCount <: Dist{(self.rank == 1)} = X10Util.get2DDistRank(dField, 0);
		val count <: DistArray[Int]{(self.rank == 1)} = (DistArray.make[Int](dCount, (pt:Point{(self.rank == 1)}) => {
			return 0;
		}));
		for (pl: Point{(self.rank == 1)} in Dist.makeUnique()) {
			...
			var offsets: DistArray[Int]{(self.rank == 1)} = (DistArray.make[Int](dLocalC));
			...
			for (i: Point{(self.rank == 1)} in rLocalP) 
				result(indices(i)) = particles(i);
		}
		return result;
	}

	public def parallelPrefix(val data: DistArray[Int]{(self.rank == 1)}): DistArray[Int]{(self.rank == 1)} {
		val dData <: Dist{(self.rank == 1)} = data.dist;
		val result <: DistArray[Int]{(self.rank == 1)} = (DistArray.make[Int](dData, (pt:Point{(self.rank == 1)}) => {
			return data(pt);
		}));
		val tmp1 <: DistArray[Int]{(self.rank == 1)} = (DistArray.make[Int](dData, (pt:Point{(self.rank == 1)}) => {
			return data(pt);
		}));
		val tmp2 <: DistArray[Int]{(self.rank == 1)} = (DistArray.make[Int](dData, (pt:Point{(self.rank == 1)}) => {
			return 0;
		}));
		var strides: Int = 1;
		while ((strides < Place.MAX_PLACES)) {
			...
			for (pl: Point{(self.rank == 1)} in Dist.makeUnique()) {
				...
				val thaValue <: Int = tmp1(pl);
				val pDst <: Place = Place.place((pl(0) + ((((dir == 0))?1:-1) * stride)));
{
					if ((dir == 0)) {

							result(here.id) = (result(here.id) + thaValue);
					}					tmp2(here.id) = thaValue;
				}			}
			for (pl: Point{(self.rank == 1)} in Dist.makeUnique()) 
...
class ArrayUtil {
	...
	public static def doubleToComplex(val src: DistArray[Double]{(self.rank == 2)}): DistArray[Complex]{(self.rank == 2)} {
		...
		if (((rS2.size() % 2) != 0)) {
{
		...
		val rD2 <: Region{(self.rank == 1)} = (0..((rS2.size() / 2) - 1));
		val dSrc <: Dist{(self.rank == 2)} = src.dist;
		val dResult <: Dist{(self.rank == 2)} = X10Util.extendDistRank2D(X10Util.get2DDistRank(dSrc, 0), rD2);
		val result <: DistArray[Complex]{(self.rank == 2)} = (DistArray.make[Complex](dResult));
		for (pt: Point{(self.rank == 2)} in dResult) 
			result(pt) = new Complex(src((pt * Point.make([1, 2]))), src(((pt * Point.make([1, 2])) + Point.make([0, 1]))));
		return result;
	}

	public static def complexToDouble(val src: DistArray[Complex]{(self.rank == 2)}): DistArray[Double]{(self.rank == 2)} {
		val rSrc <: Region{(self.rank == 2)} = src.region;
		val rS2 <: Region{(self.rank == 1)} = X10Util.getRank2D(rSrc, 1);
		val rD2 <: Region{(self.rank == 1)} = (0..((2 * rS2.size()) - 1));
		val dSrc <: Dist{(self.rank == 2)} = src.dist;
		val dResult <: Dist{(self.rank == 2)} = X10Util.extendDistRank2D(X10Util.get2DDistRank(dSrc, 0), rD2);
		val result <: DistArray[Double]{(self.rank == 2)} = (DistArray.make[Double](dResult));
		for (pt: Point{(self.rank == 2)} in dResult) {
	...
	public static def transpose(val data: DistArray[Complex]{(self.rank == 2)}): DistArray[Complex]{(self.rank == 2)} {
			...
			if ((rLocal.size() > 0)) {
{
					...
					val rLocalT <: Region{(self.rank == 2)} = (rTtmp1.min(0)..rTtmp1.max(0)) * (rTtmp2.min(0)..rTtmp2.max(0));
					val rRSBuffer <: Region{(self.rank == 1)} = (Dist.makeUnique().region as Region{(self.rank == 1)});
					val rSBuffer <: DistArray[Region{(self.rank == 2)}]{(self.rank == 1)} = ((DistArray.make[Region{(self.rank == 2)}](rRSBuffer-> here, (p:Point{(self.rank == 1)}) => {
						val rTLocal <: Region{(self.rank == 2)} = (((dResult | Dist.makeUnique()(p))).region as Region{(self.rank == 2)});
						return (rLocalT && rTLocal);
					})) as DistArray[Region{(self.rank == 2)}]{(self.rank == 1)});
					val sBuffer <: DistArray[Complex]{(self.rank == 2)} = ((DistArray.make[Complex](rLocalT-> here, ([i, j]:Point{(self.rank == 2)}) => {
						return data(j, i);
					})) as DistArray[Complex]{(self.rank == 2)});
					for (i: Point{(self.rank == 1)} in rSBuffer) 
						for (pt: Point{(self.rank == 2)} in (rSBuffer(i)->Dist.makeUnique()(i))) 
							result(pt) = sBuffer(pt);
				}			}
		}
		return result;
