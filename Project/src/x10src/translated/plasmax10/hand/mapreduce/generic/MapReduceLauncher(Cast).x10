...
class Timer {
    ...
    public def start(n: Int): Void {
        (Transfer.transfer(startTimes))(n) = System.currentTimeMillis();
    }
    ...
    public def stop(n: Int): Void {
        (Transfer.transfer(elapsedTimes))(n) =
            System.currentTimeMillis() - (Transfer.transfer(startTimes))(n);
        (Transfer.transfer(elapsedTimes))(n) /= 1000;
        (Transfer.transfer(totalTimes))(n) += (Transfer.transfer(elapsedTimes))(n);
    }
    ...
    public def readTimer(n: Int): Double {
        return (Transfer.transfer(totalTimes))(n);
    }
    ...
    public def resetTimer(n: Int): Void {
        (Transfer.transfer(totalTimes))(n) = 0;
        (Transfer.transfer(startTimes))(n) = 0;
        (Transfer.transfer(elapsedTimes))(n) = 0;
    }
        ...
        finish
            ateach (p in inputArray) {
                ...
                for(var i: Int = 0; i < intermsRail.length; i++) {
                    val interm = (Transfer.transfer(intermsRail))(i);
                    val index = interm.first.hashCode() % REDUCER_COUNT;
                    (Transfer.transfer(hereRail))(index).add(interm);
                }
            }
    }
