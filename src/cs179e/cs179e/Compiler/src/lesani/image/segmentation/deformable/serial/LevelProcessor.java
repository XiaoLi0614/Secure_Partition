...
public class LevelProcessor extends MultiStepMeshProcessor {
    ...
    public LevelProcessor(GSImage image, NodePointer first, int minEdgeLength, int maxEdgeLength) {
        super(image, first);
        this.image = image;
        DVector[][] gradient = computeGradientImage(image);

        forcer = new Forcer(image, gradient, first);
        lengthUnifier = new LengthUnifier(first, minEdgeLength, maxEdgeLength);
...
//            ImageUtil.showImage(current());
    ...
    private DVector[][] computeGradientImage(GSImage image) {
...
//        System.out.println("cosAlpha: " + cosAlpha);
        return (cosAlpha > 0.7/*7*/ - 0.001) /*&& (cosAlpha < 1)*/;
    }
